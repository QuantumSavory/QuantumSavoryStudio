...
handoff. The integration scripts own server startup, readiness, diagnostics, and
cleanup.

## Repository rules

- Prefer one clear owner for each behavior. Treat executable routes, codecs, contracts,
  callers, and tests as authoritative instead of maintaining prose inventories.
- Document the current repository only. Do not add migration paths, compatibility
  shims, or descriptions of retired behavior unless a task explicitly requires them.
