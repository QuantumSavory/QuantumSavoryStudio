...
- **Parents:** SYS-002
- **Acceptance criterion:** A discriminating current-schema fixture round-trips durable fields in the same release; simulation projection emits only explicit representations and minimized topology; script export clones only name, variables, topology, representations, and positive timing; protocol-parameter name canaries survive both projections for catalog admission; undeclared source/override fields do not cross either boundary; endpoint references hydrate into independent values; all source values remain unchanged.
