...
- **Pass criterion:** Every invalid candidate, including an inexact integer-state input, leaves source, selection, retained identities, and commit hooks unchanged; every valid candidate allocates fresh IDs, resolves aliases, distinguishes exact integers from floating state values and required values from optional omission, normalizes constructors from live metadata, preserves each asserted live identity, and updates every retained reference exactly once.
- **Status:** implemented
