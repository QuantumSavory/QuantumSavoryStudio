...
- **Pass criterion:** Projection is independent, nonmutating, and endpoint-exact; direct numeric values remain finite JSON numbers or arrays, only null omits a keyword, Function/Lambda Default aliases fail, and no backend default or legacy field is accepted; owned malformed variants and duplicate graph or result-addressing IDs fail before side effects; declared opaque values pass while unknown/nested tags fail; physical and virtual schemas are disjoint; OpenAPI closes the States Zoo wrapper, numeric map, and catalog records; runtime catalog validation rejects unknown families, keys, mistyped or fractional integer values, and out-of-range values, while browser admission also rejects unsafe integers.
- **Status:** implemented
