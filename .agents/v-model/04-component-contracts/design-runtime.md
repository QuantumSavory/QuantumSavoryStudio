...
- **Normative statement:** A design command shall run serially on an isolated candidate, admit already-typed exact JSON without coercion, validate exact live-catalog inputs before commit, allocate browser-owned durable IDs, resolve transaction-local aliases, and preserve retained live object identities during atomic reconciliation; only UI draft adapters may parse text before creating a command.
- **Parents:** SUB-003, SUB-005
- **Acceptance criterion:** Numeric strings, stringified vectors, nonfinite values, unsafe/fractional integers, legacy Default/null Variables, parameterized no-noise, unavailable or malformed catalogs, unknown slot/background/protocol/state types or protocol fields, omitted required fields, invalid protocol placement, and descriptor/value/reference conflicts leave direct, copied, or generated source design, selection, identities, and hooks unchanged. UI integer drafts validate their original decimal lexeme before conversion and opaque `Any` drafts expose only recursively untagged exact JSON after a valid commit; a command receives only finite typed numbers/arrays/objects. Valid candidates render every protocol-schema field without a local name denylist, distinguish floating from JavaScript-safe integer values and concrete Variables from optional constructor omission, normalize constructors from live metadata, allocate distinct IDs, resolve aliases, and update every retained reference once.
